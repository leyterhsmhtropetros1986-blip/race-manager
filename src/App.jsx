...
function ThreeDViewButton({gpxUrl,activityName}){
  const {lang}=useLang();
  const [show,setShow]=useState(false);
  if(!gpxUrl)return null;
  return <>
    <button onClick={()=>setShow(true)} style={{marginTop:"10px",width:"100%",background:`linear-gradient(135deg, ${T.primary} 0%, ${T.accent} 100%)`,color:"#fff",border:"none",borderRadius:"10px",padding:"10px",fontSize:"13px",fontWeight:700,cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:"8px",boxShadow:`0 4px 14px ${T.primary}33`}}>🎬 {lang==="el"?"3D Προβολή Διαδρομής":"3D Route View"}</button>
    {show&&<ThreeD3DViewer gpxUrl={gpxUrl} title={activityName} onClose={()=>setShow(false)}/>}
  </>;
}

function ThreeD3DViewer({gpxUrl,title,onClose}){
  const {lang}=useLang();
  ...
  useEffect(()=>{
    ...
    async function init(){
      try{
        ...
        // 2. Fetch and parse GPX
        const res=await fetch(gpxUrl);
        if(!res.ok)throw new Error("Cannot load GPX");
        const text=await res.text();
        const doc=new DOMParser().parseFromString(text,"text/xml");
        const trkpts=doc.querySelectorAll("trkpt");
        if(trkpts.length<2)throw new Error("No GPX points");
        ...
        trkpts.forEach(pt=>{
          ...
          const eleEl=pt.querySelector("ele");
          const ele=eleEl?parseFloat(eleEl.textContent):0;
          if(!isNaN(lat)&&!isNaN(lon))rawPoints.push({lat,lon,ele:isNaN(ele)?0:ele});
        });
        if(rawPoints.length<2)throw new Error("Insufficient points");
        
        // 3. Compute distances and bounds
        function distM(a,b){
          const R=6371000;
        ...
        for(let i=1;i<rawPoints.length;i++){
          cumDist+=distM(rawPoints[i-1],rawPoints[i]);
        ...
        const minEle=Math.min(...rawPoints.map(p=>p.ele));
        const maxEle=Math.max(...rawPoints.map(p=>p.ele));
        const eleRange=Math.max(1,maxEle-minEle);
        
        let totalGain=0,totalLoss=0;
        for(let i=1;i<rawPoints.length;i++){
          const d=rawPoints[i].ele-rawPoints[i-1].ele;
          if(d>0)totalGain+=d; else totalLoss+=Math.abs(d);
        }
        
        if(!cancelled)setStats({distance:totalKm.toFixed(2),gain:Math.round(totalGain),loss:Math.round(totalLoss),points:rawPoints.length});
        ...
        // Convert lat/lon/ele to 3D space (centered, scaled)
        const scale=80;
        const latRange=Math.max(0.0001,maxLat-minLat);
        const lonRange=Math.max(0.0001,maxLon-minLon);
        const horizontalScale=scale/Math.max(latRange,lonRange);
        const eleScale=20/eleRange;
        const centerLat=(minLat+maxLat)/2;
        const centerLon=(minLon+maxLon)/2;
        
        const points3D=rawPoints.map(p=>new THREE.Vector3(
          (p.lon-centerLon)*horizontalScale,
          (p.ele-minEle)*eleScale,
          -(p.lat-centerLat)*horizontalScale
        ));
        
        // Ground plane (semi-transparent grid)
        const gridSize=scale*1.5;
        const grid=new THREE.GridHelper(gridSize,30,0x4a5dc7,0x2a2a44);
        grid.material.opacity=0.3;
        grid.material.transparent=true;
        grid.position.y=-2;
        scene.add(grid);
        
        // Create path as tube with colored vertices by slope
        const curve=new THREE.CatmullRomCurve3(points3D);
        const tubeSegments=Math.min(points3D.length*4,400);
        const tubeGeo=new THREE.TubeGeometry(curve,tubeSegments,0.4,8,false);
        
        // Color by slope: compute slope at each vertex
        const colors=[];
        const tubeVertCount=tubeGeo.attributes.position.count;
        for(let i=0;i<tubeVertCount;i++){
          const t=i/(tubeVertCount-1);
          const idx=Math.min(points3D.length-1,Math.floor(t*(points3D.length-1)));
          const prevIdx=Math.max(0,idx-1);
          const slope=(points3D[idx].y-points3D[prevIdx].y);
          let r,g,b;
          if(slope>0.3){r=1;g=0.3;b=0.2;}      // Steep climb - red
          else if(slope>0.1){r=1;g=0.7;b=0.2;}  // Moderate climb - orange
          else if(slope>-0.1){r=0.3;g=0.9;b=0.4;} // Flat - green
          else if(slope>-0.3){r=0.3;g=0.7;b=1;}   // Downhill - blue
          else {r=0.6;g=0.4;b=1;}                  // Steep downhill - purple
          colors.push(r,g,b);
        }
        tubeGeo.setAttribute("color",new THREE.Float32BufferAttribute(colors,3));
        const tubeMat=new THREE.MeshPhongMaterial({vertexColors:true,shininess:80,specular:0x222222});
        const tube=new THREE.Mesh(tubeGeo,tubeMat);
        scene.add(tube);
        
        // Drop lines from path to ground for depth perception
        const lineMat=new THREE.LineBasicMaterial({color:0x4a5dc7,transparent:true,opacity:0.15});
        for(let i=0;i<points3D.length;i+=Math.max(1,Math.floor(points3D.length/40))){
          const lineGeo=new THREE.BufferGeometry().setFromPoints([
            new THREE.Vector3(points3D[i].x,points3D[i].y,points3D[i].z),
            new THREE.Vector3(points3D[i].x,-2,points3D[i].z)
          ]);
        ...
        const startMat=new THREE.MeshPhongMaterial({color:0x10b981,emissive:0x064e3b});
        const startMesh=new THREE.Mesh(startGeo,startMat);
        startMesh.position.copy(points3D[0]);
        scene.add(startMesh);
        
        // End marker (red sphere)
        const endGeo=new THREE.SphereGeometry(1.2,16,16);
        const endMat=new THREE.MeshPhongMaterial({color:0xef4444,emissive:0x7f1d1d});
        const endMesh=new THREE.Mesh(endGeo,endMat);
        endMesh.position.copy(points3D[points3D.length-1]);
        scene.add(endMesh);
        
        // KM markers
        const totalDistM=cumDist;
        const kmCount=Math.floor(totalDistM/1000);
        for(let k=1;k<=kmCount;k++){
          ...
          const markerGeo=new THREE.ConeGeometry(0.6,2.5,8);
          const markerMat=new THREE.MeshPhongMaterial({color:0xfbbf24,emissive:0x78350f});
          const marker=new THREE.Mesh(markerGeo,markerMat);
          marker.position.copy(points3D[idx]);
          marker.position.y+=2.5;
          scene.add(marker);
        }
        
        // Custom orbit controls (mouse drag rotate, wheel zoom)
        ...
        function updateCamera(){
          ...
          camera.position.y=Math.sin(rotationX)*cameraDistance+10;
        ...
        function onMouseMove(e){
          ...
          rotationY-=dx*0.01;
          rotationX=Math.max(0.1,Math.min(Math.PI/2-0.1,rotationX+dy*0.01));
          prevX=e.clientX;
          prevY=e.clientY;
          updateCamera();
        }
        function onWheel(e){
          e.preventDefault();
          cameraDistance=Math.max(30,Math.min(250,cameraDistance+e.deltaY*0.1));
          updateCamera();
        }
        function onTouchStart(e){if(e.touches.length===1){isDragging=true;prevX=e.touches[0].clientX;prevY=e.touches[0].clientY;}}
        ...
        let autoRotate=true;
        dom.addEventListener("mousedown",()=>autoRotate=false);
        dom.addEventListener("touchstart",()=>autoRotate=false);
        
        let animationId;
        function animate(){
          if(cancelled)return;
          animationId=requestAnimationFrame(animate);
          if(autoRotate){rotationY+=0.002;updateCamera();}
  ...
  return <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.85)",zIndex:9999,display:"flex",flexDirection:"column",padding:"20px"}}>
    {/* Header */}
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"12px",color:"#fff",flexWrap:"wrap",gap:"10px"}}>
      <div>
        <div style={{fontSize:"16px",fontWeight:800}}>🎬 {title||"3D Route"}</div>
        {stats&&<div style={{fontSize:"12px",opacity:0.8,marginTop:"2px"}}>📏 {stats.distance}km · ⛰ +{stats.gain}m / -{stats.loss}m</div>}
      </div>
      <button onClick={onClose} style={{background:"rgba(255,255,255,0.15)",color:"#fff",border:"1px solid rgba(255,255,255,0.3)",borderRadius:"10px",padding:"8px 16px",fontSize:"13px",fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>✕ {lang==="el"?"Κλείσιμο":"Close"}</button>
    </div>
    
    {/* 3D Canvas Container */}
    <div ref={containerRef} style={{flex:1,background:"#1a1a2e",borderRadius:"14px",overflow:"hidden",position:"relative",minHeight:"300px"}}>
      {loading&&!error&&<div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:"14px"}}>🌐 {lang==="el"?"Φόρτωση 3D...":"Loading 3D..."}</div>}
      {error&&<div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:"14px",padding:"20px",textAlign:"center"}}>⚠️ {error}</div>}
    </div>
    
    {/* Legend */}
    <div style={{display:"flex",gap:"10px",justifyContent:"center",flexWrap:"wrap",marginTop:"10px",fontSize:"11px",color:"#fff",opacity:0.9}}>
      <span><span style={{display:"inline-block",width:"10px",height:"10px",background:"#ff4d33",borderRadius:"2px",marginRight:"4px"}}></span>{lang==="el"?"Απότομη ανηφόρα":"Steep climb"}</span>
      <span><span style={{display:"inline-block",width:"10px",height:"10px",background:"#ffb133",borderRadius:"2px",marginRight:"4px"}}></span>{lang==="el"?"Ανηφόρα":"Climb"}</span>
      <span><span style={{display:"inline-block",width:"10px",height:"10px",background:"#4de666",borderRadius:"2px",marginRight:"4px"}}></span>{lang==="el"?"Επίπεδο":"Flat"}</span>
      <span><span style={{display:"inline-block",width:"10px",height:"10px",background:"#4db3ff",borderRadius:"2px",marginRight:"4px"}}></span>{lang==="el"?"Κατηφόρα":"Downhill"}</span>
      <span>🟢 {lang==="el"?"Start":"Start"} · 🔴 {lang==="el"?"End":"End"} · 🔺 KM</span>
    </div>
    <div style={{textAlign:"center",fontSize:"10px",color:"#fff",opacity:0.6,marginTop:"6px"}}>💡 {lang==="el"?"Σύρε για περιστροφή, scroll για zoom":"Drag to rotate, scroll to zoom"}</div>
  </div>;
}
          ...
          <div style={{display:"flex",flexDirection:"column",gap:"8px"}}>
            {(showAllActivities?activities.slice(0,10):activities.slice(0,1)).map(act=>{
              ...
              return <div key={act.id} style={{background:T.bg,borderRadius:"10px",padding:"12px 14px",border:`1px solid ${T.border}`}}>
                ...
                {act.gpx_url&&<ThreeDViewButton gpxUrl={act.gpx_url} activityName={act.name}/>}
