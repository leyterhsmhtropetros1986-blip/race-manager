...
// ─── ΧΡΩΜΑΤΙΚΟ THEME — LIGHT (γκρι/μπεζ) ───────────────────────────────────────
const T = {
  bg: "#f5f3ef",           // κύριο φόντο (μπεζ ανοιχτό)
  bgAlt: "#ffffff",        // κάρτες/modals (λευκό)
  bgInput: "#ffffff",      // πεδία input
  border: "#e0ddd6",       // περιγράμματα ανοιχτά
  borderDark: "#c9c5bc",   // πιο σκούρα περιγράμματα
  text: "#2a2a2e",         // κύριο κείμενο (σχεδόν μαύρο)
  textMid: "#6b6b73",      // δευτερεύον κείμενο
  textLight: "#9a9aa3",    // πολύ ξεθωριασμένο
  primary: "#4a5dc7",      // μπλε (διοργανωτής)
  primaryDark: "#3a4ba8",
  accent: "#2da77f",       // πράσινο (αθλητής)
  accentDark: "#1f8862",
  warning: "#d4a017",      // κίτρινο/πορτοκαλί
  danger: "#d04545",       // κόκκινο
...
function Modal({title,onClose,children,wide}){
  return <div style={{position:"fixed",inset:0,background:"rgba(40,40,50,0.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999,padding:"20px"}}>
    <div style={{background:T.bgAlt,border:`1px solid ${T.border}`,borderRadius:"16px",padding:"28px",width:"100%",maxWidth:wide?"700px":"520px",maxHeight:"88vh",overflowY:"auto",boxShadow:"0 10px 40px rgba(0,0,0,0.15)"}}>
...
function DistancesPicker({distances,onChange}){
  ...
  return <F label="Διαδρομές / Αποστάσεις *">
    ...
    {distances.length>0&&(
      <div>
        <div style={{color:T.textMid,fontSize:"11px",marginBottom:"6px"}}>Επιλεγμένες:</div>
        <div style={{display:"flex",gap:"6px",flexWrap:"wrap"}}>
          {distances.map((d,i)=>(
        ...
        <div>
          ...
          <button onClick={()=>selectRole("organizer")} style={{width:"100%",background:T.primary,color:"#fff",border:"none",borderRadius:"12px",padding:"20px",fontSize:"14px",fontWeight:700,cursor:"pointer",fontFamily:"inherit",marginBottom:"12px",textAlign:"left",display:"flex",alignItems:"center",gap:"16px",boxShadow:T.shadow}}>
            ...
            <div>
              ...
              <div style={{fontSize:"12px",fontWeight:400,opacity:0.9}}>Δημιουργώ & διαχειρίζομαι αγώνες</div>
            </div>
          </button>
          ...
          <button onClick={()=>selectRole("athlete")} style={{width:"100%",background:T.accent,color:"#fff",border:"none",borderRadius:"12px",padding:"20px",fontSize:"14px",fontWeight:700,cursor:"pointer",fontFamily:"inherit",textAlign:"left",display:"flex",alignItems:"center",gap:"16px",boxShadow:T.shadow}}>
            ...
            <div>
              ...
              <div style={{fontSize:"12px",fontWeight:400,opacity:0.9}}>Βλέπω αγώνες & εγγράφομαι</div>
            </div>
          </button>
        </div>
      )}
      ...
      {step==="auth"&&(
        <div>
          <div style={{textAlign:"center",marginBottom:"20px"}}>
            <span style={{background:`${roleColor}15`,color:roleColor,border:`1px solid ${roleColor}44`,borderRadius:"99px",padding:"6px 16px",fontSize:"13px",fontWeight:700,display:"inline-flex",alignItems:"center",gap:"6px"}}>
              {roleIcon} {roleLabel}
            </span>
          </div>

          <div style={{display:"flex",background:T.bg,borderRadius:"10px",padding:"4px",marginBottom:"24px",border:`1px solid ${T.border}`}}>
            <button onClick={()=>{setMode("signup");setError("");}} style={{flex:1,background:mode==="signup"?roleColor:"none",color:mode==="signup"?roleColorText:T.textMid,border:"none",borderRadius:"8px",padding:"10px",fontSize:"13px",fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
              ✨ Νέα Εγγραφή
            </button>
            <button onClick={()=>{setMode("login");setError("");}} style={{flex:1,background:mode==="login"?roleColor:"none",color:mode==="login"?roleColorText:T.textMid,border:"none",borderRadius:"8px",padding:"10px",fontSize:"13px",fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
              🔑 Σύνδεση
            </button>
          </div>

          <p style={{color:T.textMid,fontSize:"12px",textAlign:"center",marginBottom:"20px",lineHeight:"1.5"}}>
            {mode==="signup"
          ...
          {mode==="signup"&&role==="organizer"&&(
            <div style={{background:`${T.warning}15`,border:`1px solid ${T.warning}44`,borderRadius:"8px",padding:"10px 14px",color:T.warning,fontSize:"12px",marginBottom:"16px",lineHeight:"1.5"}}>
              ⚠️ Ο λογαριασμός Διοργανωτή χρειάζεται έγκριση από admin πριν ενεργοποιηθεί.
            </div>
          )}

          {mode==="signup"&&(
            <div style={{marginBottom:"14px"}}>
              <label style={css.label}>Ονοματεπώνυμο</label>
          ...
          <div style={{marginBottom:"20px"}}>
            <label style={css.label}>Κωδικός</label>
          ...
          <button onClick={handleSubmit} disabled={loading} style={{width:"100%",background:roleColor,color:roleColorText,border:"none",borderRadius:"10px",padding:"14px",fontSize:"14px",fontWeight:700,cursor:"pointer",fontFamily:"inherit",marginBottom:"12px",opacity:loading?0.6:1,boxShadow:T.shadow}}>
            {loading?"...":(mode==="signup"?`✨ Εγγραφή ως ${roleLabel}`:`🔑 Σύνδεση`)}
          </button>

          <button onClick={backToRole} style={{width:"100%",background:"none",border:`1px solid ${T.border}`,color:T.textMid,borderRadius:"10px",padding:"10px",fontSize:"13px",cursor:"pointer",fontFamily:"inherit"}}>
            ← Αλλαγή ρόλου
          </button>
        </div>
      )}
  ...
  return <div style={{background:T.bgAlt,border:`1px solid ${T.border}`,borderRadius:"14px",padding:"20px 24px",boxShadow:T.shadow}}>
    ...
    {distances.length>0&&(
      <div style={{display:"flex",gap:"6px",flexWrap:"wrap",marginBottom:"12px"}}>
        {distances.map((d,i)=>(
          <span key={i} style={{background:`${T.primary}12`,border:`1px solid ${T.primary}33`,borderRadius:"6px",padding:"3px 10px",fontSize:"12px",color:T.primary,fontWeight:500}}>🏃 {d}</span>
        ))}
      </div>
    )}
    {myReg?(
    ...
    <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
      {myRaces.map(race=>{
        ...
        return <div key={race.id} style={{background:T.bgAlt,border:`1px solid ${T.border}`,borderRadius:"14px",padding:"20px 24px",boxShadow:T.shadow}}>
          ...
          <div style={{color:T.textMid,fontSize:"13px",lineHeight:"1.8",marginBottom:"10px"}}>📅 {race.date} &nbsp; 📍 {race.location||"—"} &nbsp; 👤 {regCount} εγγεγραμμένοι</div>
          {distances.length>0&&(
            <div style={{display:"flex",gap:"6px",flexWrap:"wrap",marginBottom:"12px"}}>
              {distances.map((d,i)=>(<span key={i} style={{background:`${T.primary}12`,border:`1px solid ${T.primary}33`,borderRadius:"6px",padding:"3px 10px",fontSize:"12px",color:T.primary,fontWeight:500}}>🏃 {d}</span>))}
            </div>
          )}
...
function OrganizerRegistrations({races,runners,registrations,session,profile}){
  ...
  return <div>
    <h2 style={{margin:"0 0 20px",color:T.text,fontSize:"20px"}}>Εγγραφές στους Αγώνες μου ({filtered.length})</h2>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:"8px"}}>
      {filtered.map(reg=>{
        ...
        return <div key={reg.id} style={{background:T.bgAlt,border:`1px solid ${T.border}`,borderRadius:"12px",padding:"14px 18px",display:"flex",alignItems:"center",gap:"12px",boxShadow:T.shadow}}>
          ...
          <div style={{flex:1}}>
            <div style={{color:T.text,fontWeight:700,fontSize:"14px"}}>{runner.first_name} {runner.last_name}</div>
...
function AdminPanel(){
  ...
  async function fetchOrgs(){
    ...
    if(data){
      setPendingOrgs(data.filter(o=>o.status==="pending"));
      setAllOrgs(data);
    }
    setLoading(false);
  }
  ...
  async function reject(id){
    if(!confirm("Σίγουρα θέλετε να απορρίψετε αυτόν τον διοργανωτή;"))return;
    await supabase.from("profiles").update({status:"rejected"}).eq("id",id);
    fetchOrgs();
  }

  async function makeAdmin(id){
    if(!confirm("Να γίνει admin αυτός ο χρήστης;"))return;
    await supabase.from("profiles").update({role:"admin",status:"approved"}).eq("id",id);
    fetchOrgs();
  }
  ...
  return <div>
    <div style={{display:"flex",gap:"6px",marginBottom:"24px",flexWrap:"wrap"}}>
      ...
      <button onClick={()=>setTab("all")} style={{background:tab==="all"?T.primary:T.bgAlt,color:tab==="all"?"#fff":T.textMid,border:`1px solid ${tab==="all"?T.primary:T.border}`,borderRadius:"8px",padding:"10px 18px",cursor:"pointer",fontSize:"13px",fontWeight:tab==="all"?700:500,fontFamily:"inherit"}}>👥 Όλοι οι Διοργανωτές ({allOrgs.length})</button>
    </div>

    <h2 style={{margin:"0 0 16px",color:T.text,fontSize:"18px"}}>
      {tab==="pending"?"Διοργανωτές σε Αναμονή Έγκρισης":"Όλοι οι Διοργανωτές"}
    </h2>

    {list.length===0&&<div style={{textAlign:"center",color:T.textLight,padding:"60px",fontSize:"14px"}}>{tab==="pending"?"🎉 Δεν υπάρχουν διοργανωτές σε αναμονή!":"Δεν υπάρχουν διοργανωτές"}</div>}
    ...
    <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
      {list.map(org=>(
        <div key={org.id} style={{background:T.bgAlt,border:`1px solid ${T.border}`,borderRadius:"14px",padding:"20px 24px",boxShadow:T.shadow}}>
          ...
          <div style={{display:"flex",gap:"8px",flexWrap:"wrap"}}>
            {org.status==="pending"&&<>
              <Btn sm v="grn" onClick={()=>approve(org.id)}>✅ Έγκριση</Btn>
              <Btn sm v="red" onClick={()=>reject(org.id)}>❌ Απόρριψη</Btn>
            </>}
...
export default function App(){
  ...
  return <div style={{minHeight:"100vh",background:T.bg,color:T.text,fontFamily:"Inter,sans-serif"}}>
    ...
    {isPendingOrganizer&&(
      <div style={{padding:"40px 28px",maxWidth:"640px",margin:"40px auto"}}>
        <div style={{background:T.bgAlt,border:`1px solid ${T.warning}44`,borderRadius:"16px",padding:"40px",textAlign:"center",boxShadow:T.shadow}}>
          ...
          <p style={{color:T.text,fontSize:"15px",lineHeight:"1.7",margin:"0 0 8px"}}>
            Ο λογαριασμός σας ως <strong>Διοργανωτής</strong> έχει δημιουργηθεί επιτυχώς!
          </p>
          <p style={{color:T.textMid,fontSize:"13px",lineHeight:"1.7",margin:"0 0 24px"}}>
            Πρέπει πρώτα να εγκριθεί από τον διαχειριστή της πλατφόρμας για να μπορείτε να δημιουργείτε αγώνες.
          </p>
    ...
    {isRejectedOrganizer&&(
      <div style={{padding:"40px 28px",maxWidth:"640px",margin:"40px auto"}}>
        <div style={{background:T.bgAlt,border:`1px solid ${T.danger}44`,borderRadius:"16px",padding:"40px",textAlign:"center",boxShadow:T.shadow}}>
          ...
          <p style={{color:T.textMid,fontSize:"13px",lineHeight:"1.7",margin:"0 0 24px"}}>
            Δυστυχώς η αίτησή σας για ρόλο Διοργανωτή δεν εγκρίθηκε. Επικοινωνήστε με τον διαχειριστή.
          </p>
          <button onClick={()=>supabase.auth.signOut()} style={{background:T.bgAlt,color:T.text,border:`1px solid ${T.border}`,borderRadius:"8px",padding:"10px 20px",cursor:"pointer",fontSize:"13px",fontFamily:"inherit"}}>Αποσύνδεση</button>
        </div>
      </div>
